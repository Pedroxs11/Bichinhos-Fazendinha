...
class MimosaCarePanel(
    ...
    private val happiness = stat("❤️ Felicidade")
    ...
    init {
        ...
        reaction.gravity = Gravity.CENTER
        reaction.textSize = 18f
        reaction.setTextColor(0xFF4A392D.toInt())
        reaction.text = "❤️ Muuu! Estou feliz!"
        coins.gravity = Gravity.CENTER
        coins.textSize = 18f
        coins.setTextColor(0xFF4A392D.toInt())
        addView(coins, lp())
        addView(hunger.root, lp())
        addView(hygiene.root, lp())
        addView(happiness.root, lp())
    ...
    private fun stat(label: String): Stat {
        val row = LinearLayout(context).apply { orientation = HORIZONTAL; gravity = Gravity.CENTER_VERTICAL; setPadding(12, 5, 12, 5) }
        val name = TextView(context).apply { text = label; textSize = 16f; setTextColor(0xFF3F332B.toInt()) }
        val bar = ProgressBar(context, null, android.R.attr.progressBarStyleHorizontal).apply { max = 100 }
        val value = TextView(context).apply { textSize = 15f; gravity = Gravity.END; setTextColor(0xFF3F332B.toInt()) }
        row.addView(name, LayoutParams(0, -2, 1.25f))
        row.addView(bar, LayoutParams(0, 24, 1.5f).apply { setMargins(8, 0, 8, 0) })
        row.addView(value, LayoutParams(0, -2, .55f))
    ...
    private fun refresh() {
        hunger.bar.progress = state.hunger; hunger.value.text = "${state.hunger}%"
        hygiene.bar.progress = state.hygiene; hygiene.value.text = "${state.hygiene}%"
        happiness.bar.progress = state.happiness; happiness.value.text = "${state.happiness}%"
        energy.bar.progress = state.energy; energy.value.text = "${state.energy}%"
        coins.text = "🐮 Mimosa • Nível 1     🪙 ${state.coins}"
    }

    private fun lp() = LayoutParams(-1, -2).apply { setMargins(0, 4, 0, 4) }
    private fun weight() = LayoutParams(0, 112, 1f).apply { setMargins(4, 4, 4, 4) }
}
