...
class Cow3DView @JvmOverloads constructor(context:Context,attrs:AttributeSet?=null):View(context,attrs){
 ...
 override fun onDraw(c:Canvas){super.onDraw(c);val w=width.toFloat();val h=height.toFloat();val s=minOf(w,h)*.22f;val cx=w*.49f;val cy=h*.58f
  c.drawColor(if(careState==3)Color.rgb(72,93,145) else Color.rgb(166,224,255));if(careState!=3){p.color=0xFFFFE36E.toInt();c.drawCircle(w*.84f,h*.14f,s*.25f,p)}else{p.color=0xFFFFF3B0.toInt();c.drawCircle(w*.84f,h*.14f,s*.23f,p);p.color=0xFF485D91.toInt();c.drawCircle(w*.9f,h*.1f,s*.23f,p)};p.color=Color.WHITE;cloud(c,w*.16f,h*.16f,s);if(careState!=3)cloud(c,w*.7f,h*.25f,s*.72f)
  p.color=if(careState==3)0xFF467047.toInt() else 0xFF76C95A.toInt();c.drawRect(0f,h*.66f,w,h,p);p.color=if(careState==3)0xFF3C623E.toInt() else 0xFF5FA947.toInt();c.drawOval(-w*.1f,h*.6f,w*.6f,h*.82f,p);c.drawOval(w*.42f,h*.61f,w*1.1f,h*.82f,p)
