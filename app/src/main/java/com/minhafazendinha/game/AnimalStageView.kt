...
class AnimalStageView @JvmOverloads constructor(context:Context,attrs:AttributeSet?=null):FrameLayout(context,attrs){
    ...
    fun setSoundBoardMode(enabled:Boolean){soundBoardMode=enabled}
    private fun startIdle(){idle?.cancel();idle=ObjectAnimator.ofFloat(animal,"translationY",baseY-4f,baseY+6f,baseY-4f).apply{duration=1900;repeatCount=ObjectAnimator.INFINITE;start()}}
    fun setAnimal(emoji:String,accessory:String=""){
        animal.text=emoji+accessory
        if(soundBoardMode){
            animal.textSize=when(emoji){"🐑"->138f;"🐮"->114f;else->112f}
            baseY=when(emoji){"🐑"->-82f;"🐮"->-8f;else->-8f}
            shadow.translationY=when(emoji){"🐑"->28f;else->72f}
        }else{
            animal.textSize=110f;baseY=0f;shadow.translationY=78f
        }
        animal.translationY=baseY
