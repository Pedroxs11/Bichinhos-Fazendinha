...
class GameSceneView(context: Context): View(context){
    ...
    fun setMode(v:String){sceneMode=v;if(v=="lake"){if(!lakeAnimator.isStarted)lakeAnimator.start()}else lakeAnimator.cancel();invalidate()}
    ...
    private fun drawFish(c:Canvas,x:Float,y:Float,scale:Float){
        val s=scale*.06f;p.color=Color.rgb(255,154,42);c.drawOval(x-s,y-s*.45f,x+s,y+s*.45f,p)
        val tail=Path();tail.moveTo(x-s,y);tail.lineTo(x-s*1.65f,y-s*.65f);tail.lineTo(x-s*1.65f,y+s*.65f);tail.close();c.drawPath(tail,p)
        p.color=Color.WHITE;c.drawCircle(x+s*.45f,y-s*.10f,s*.14f,p);p.color=Color.rgb(45,61,70);c.drawCircle(x+s*.49f,y-s*.10f,s*.07f,p)
    }
    private fun drawDuck(c:Canvas,x:Float,y:Float,scale:Float){
        val s=scale*.055f;p.color=Color.rgb(255,242,201);c.drawOval(x-s*1.25f,y-s*.45f,x+s*1.25f,y+s*.65f,p);c.drawCircle(x+s*.70f,y-s*.65f,s*.58f,p)
        p.color=Color.rgb(255,166,37);val beak=Path();beak.moveTo(x+s*1.20f,y-s*.68f);beak.lineTo(x+s*1.75f,y-s*.48f);beak.lineTo(x+s*1.18f,y-s*.32f);beak.close();c.drawPath(beak,p)
        p.color=Color.rgb(45,61,70);c.drawCircle(x+s*.86f,y-s*.78f,s*.09f,p)
    }
    ...
    private fun drawBarn(c:Canvas,w:Float,h:Float){
        rr(c,Color.rgb(205,72,55),w*.66f,h*.34f,w*.91f,h*.68f,10f)
        p.color=Color.rgb(135,54,43);val roof=Path();roof.moveTo(w*.62f,h*.37f);roof.lineTo(w*.785f,h*.22f);roof.lineTo(w*.95f,h*.37f);roof.close();c.drawPath(roof,p)
        rr(c,Color.rgb(104,62,43),w*.745f,h*.49f,w*.835f,h*.68f,5f)
        p.color=Color.WHITE;p.strokeWidth=5f;p.style=Paint.Style.STROKE;c.drawLine(w*.75f,h*.50f,w*.83f,h*.67f,p);c.drawLine(w*.83f,h*.50f,w*.75f,h*.67f,p);p.style=Paint.Style.FILL
    }
    private fun drawTree(c:Canvas,w:Float,h:Float){
        rr(c,Color.rgb(116,73,43),w*.10f,h*.45f,w*.14f,h*.70f,8f)
        p.color=Color.rgb(55,151,70);c.drawCircle(w*.12f,h*.40f,w*.075f,p);c.drawCircle(w*.075f,h*.45f,w*.055f,p);c.drawCircle(w*.17f,h*.45f,w*.06f,p)
    }
    ...
    private fun drawLakeBubbles(c:Canvas,x:Float,y:Float,scale:Float,alpha:Float){
        p.style=Paint.Style.STROKE;p.strokeWidth=3f;p.color=Color.argb((190*alpha).toInt(),255,255,255)
        val rise=scale*.07f*(1f-alpha);val s=scale*.012f
        c.drawCircle(x-s*2.2f,y-rise,s*.55f,p);c.drawCircle(x+s*1.4f,y-rise-s*1.6f,s*.75f,p);c.drawCircle(x+s*3.1f,y-rise+s*.4f,s*.42f,p)
        p.style=Paint.Style.FILL;p.alpha=255
    }

    private fun drawLakeHeart(c:Canvas,x:Float,y:Float,scale:Float,alpha:Float){
        val s=scale*.018f;p.alpha=(255*alpha).toInt();p.color=Color.rgb(255,92,145)
        c.drawCircle(x-s*.55f,y,s*.62f,p);c.drawCircle(x+s*.55f,y,s*.62f,p)
        val heart=Path();heart.moveTo(x-s*1.15f,y+s*.15f);heart.lineTo(x,y+s*1.35f);heart.lineTo(x+s*1.15f,y+s*.15f);heart.close();c.drawPath(heart,p);p.alpha=255
    }

    override fun onTouchEvent(event:MotionEvent):Boolean{
        if(sceneMode!="lake") return super.onTouchEvent(event)
        if(event.action==MotionEvent.ACTION_DOWN){
            splashX=(event.x/width.coerceAtLeast(1)).coerceIn(.08f,.92f)
            splashY=(event.y/height.coerceAtLeast(1)).coerceIn(.52f,.90f)
            lakeTapCount++
            wildlifeBoost=(wildlifeBoost+.11f)%1f
            lakeHearts=1f
            lakeBubbles=1f
            ValueAnimator.ofFloat(1f,0f).apply{duration=900;addUpdateListener{lakeHearts=it.animatedValue as Float;invalidate()};start()}
            ValueAnimator.ofFloat(1f,0f).apply{duration=1100;addUpdateListener{lakeBubbles=it.animatedValue as Float;invalidate()};start()}
            ValueAnimator.ofFloat(1f,0f).apply{duration=650;addUpdateListener{splashAlpha=it.animatedValue as Float;invalidate()};start()}
            animate().scaleX(1.008f).scaleY(1.008f).setDuration(70).withEndAction{animate().scaleX(1f).scaleY(1f).setDuration(110).start()}.start()
            performClick();return true
        }
        return true
    }
    override fun performClick():Boolean{super.performClick();return true}
    override fun onDetachedFromWindow(){lakeAnimator.cancel();super.onDetachedFromWindow()}
