...
class GameSceneView(context: Context): View(context){
    ...
    override fun onDraw(c:Canvas){super.onDraw(c);val w=width.toFloat();val h=height.toFloat();if(w<=0)return
        ...
        when(sceneMode){
            ...
            "lake"->{oval(c,Color.rgb(78,181,226),w*.08f,h*.52f,w*.92f,h*.90f);oval(c,Color.rgb(105,213,237),w*.16f,h*.60f,w*.83f,h*.82f);drawFish(c,w*.48f,h*.72f,w);drawFish(c,w*.67f,h*.78f,w*.72f);drawDuck(c,w*.30f,h*.64f,w);drawLily(c,w*.76f,h*.68f,w);drawLakeRipples(c,w*.30f,h*.70f,w);drawLakeRipples(c,w*.53f,h*.77f,w)}
